...
def get_files(base_dir, reg_ex, in_depth=True, include_symlink=False, return_relative=True):
	...
	# for all file types specified walk into the search directory and add the list of files to file_path_list
...
#TODO: Show identical side by side and ask for option to delete them ( or just move the duplicates to be deleted to a new folder)
#TODO: Calculate similarity at a lower dimensional subspace
#TODO: Saving file (exporting not renaming) as different format changes the hash. It needs to remain the same
...
if __name__ == "__main__":
	...
	files = get_files(base_dir=PATH_TO_SEARCH, reg_ex=SEARCH_REGEX,
		in_depth=False, include_symlink=FOLLOW_SYM_LINKS, return_relative=True)
